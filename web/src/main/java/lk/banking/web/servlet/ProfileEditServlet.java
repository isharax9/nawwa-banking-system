...
import lk.banking.core.util.ValidationUtils; // Import ValidationUtils
...
public class ProfileEditServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!loggedInUser.hasRole(lk.banking.core.entity.enums.UserRole.CUSTOMER)) {
            ...
            request.getSession().setAttribute("errorMessage", "Access denied. Only customers can edit profiles.");
        ...
        try {
            ...
            request.setAttribute("customer", customer); // Pass customer object to JSP for pre-filling form
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // --- Server-side input validation using ValidationUtils ---
        ...
        } else if (!ValidationUtils.isValidPhoneNumber(phoneNumber)) { // Use ValidationUtils for phone number
        ...
        if (errorMessage != null) {
            ...
            // Keep current customer object in request to pre-fill valid fields
            try {
                ...
                // If customer not found, we can't pre-fill, rely on user re-entering. Log this.
        ...
        try {
            ...
            CustomerDto customerDto = new CustomerDto(
                    customer.getId(),
                    name,
                    customer.getEmail(), // Email is not editable via this form
                    address,
                    phoneNumber
            );
            ...
            request.getSession().setAttribute("successMessage", "Your profile has been updated successfully!");
            response.sendRedirect(request.getContextPath() + "/dashboard");

        } catch (ValidationException e) {
            LOGGER.log(java.util.logging.Level.WARNING, "ProfileEditServlet: Validation error during profile update for user: " + loggedInUser.getUsername(), e);
            request.setAttribute("errorMessage", e.getMessage());
            try { // Re-populate form with potentially valid original data
                request.setAttribute("customer", customerService.getCustomerByEmail(loggedInUser.getEmail()));
            } catch (CustomerNotFoundException ignore) { LOGGER.severe("Customer not found for re-populating after validation error"); }
            doGet(request, response);
        } catch (CustomerNotFoundException e) {
            LOGGER.log(java.util.logging.Level.WARNING, "ProfileEditServlet: Customer not found during update for user: " + loggedInUser.getUsername(), e);
            request.setAttribute("errorMessage", e.getMessage());
            try { // Re-populate form
                request.setAttribute("customer", customerService.getCustomerByEmail(loggedInUser.getEmail()));
            } catch (CustomerNotFoundException ignore) { LOGGER.severe("Customer not found for re-populating after CustomerNotFound"); }
            doGet(request, response);
        } catch (Exception e) {
            LOGGER.log(java.util.logging.Level.SEVERE, "ProfileEditServlet: An unexpected error occurred during profile update for user " + loggedInUser.getUsername() + ".", e);
            request.setAttribute("errorMessage", "An unexpected error occurred. Please try again later.");
            try { // Re-populate form
                request.setAttribute("customer", customerService.getCustomerByEmail(loggedInUser.getEmail()));
            } catch (CustomerNotFoundException ignore) { LOGGER.severe("Customer not found for re-populating after unexpected error"); }
            doGet(request, response);
        }
    }
}